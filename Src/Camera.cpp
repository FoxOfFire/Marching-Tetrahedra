...
void Camera::SetProj(float _angle, float _aspect, float _zn, float _zf,bool _perspective)
{
	m_angle  = _angle;
	m_aspect = _aspect;
	m_zNear  = _zn;
	...
	if(_perspective){
		...
		m_projMatrix = glm::ortho(-sinf(_angle)*10,sinf(_angle)*10, -sinf(_angle)/_aspect*10,sinf(_angle)/_aspect*10,_zn,_zf );
	}
}
